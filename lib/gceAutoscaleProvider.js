 ...
 *                                                     Required for cluster and autoscale solutions.
 * @param {String}  [providerOptions.secretId]       - Rest storage ID of encrypted credentials.
 *                                                     Required if not running in Google Cloud.
 * @param {String}  [providerOptions.clientId]       - Google Cloud Client ID.
 *                                                     Required if not running in Google Cloud.
...
GceAutoscaleProvider.prototype.init = function init(providerOptions, options) {
    ...
    if (this.providerOptions.secretId) {
        if (!this.providerOptions.clientId) {
            return q.reject(new Error('providerOptions.clientId is required when providing a secretId'));
        }
    }
    ...
    return getMetadata('project/project-id')
        .then((data) => {
            ...
            if (providerOptions.secretId) {
                credentialsPromise = getSecret(
                    providerOptions.secretId,
...
function getSecret(secretId, clientId, projectId) {
