 ...
 * @param {String}  [providerOptions.zone]    - Zone to use for searching instances.
 *
...
GceAutoscaleProvider.prototype.init = function(providerOptions) {
    ...
    // If we weren't given a zone, get zone we are in from metadata service
    this.zone = providerOptions.zone;
    ...
    if (!this.zone) {
        return cloudUtil.getDataFromUrl('http://metadata.google.internal/computeMetadata/v1/instance/zone', options)
            .then(function(data) {
                ...
                // In a zone, -a, -b, etc can talk to each other, so grab the part up to the last '-'
                this.zone = getShortZone(zone);

                logger.silly('zone:', this.zone);
            }.bind(this));
    }
    else {
        logger.silly('providerOptions.zone:', this.zone);
        return q();
    }
};
    ...
    this.compute.getVMs(options)
        .then(function(data) {
            ...
            data[0].forEach(function(vm) {
                if (getShortZone(vm.zone.id) === this.zone) {
...
/**
 * Gets the sub-zone for a zone.
 *
 * For example, 'us-west1-a' returns 'us-west1'
 */
var getShortZone = function(zone) {
