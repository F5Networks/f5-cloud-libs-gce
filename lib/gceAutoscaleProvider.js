...
util.inherits(GceAutoscaleProvider, AutoscaleProvider);
...
GceAutoscaleProvider.prototype.init = function init(providerOptions, options) {
    this.mgmtPort =
    ...
    if (providerOptions.secret) {
        ...
        let credentials;
        try {
            const credentialsBuf = Buffer.from(providerOptions.secret, 'base64');
 ...
 * @returns {Promise} A promise which will be resolved when init is complete.
 */
GceAutoscaleProvider.prototype.bigIpReady = function bigIpReady() {
 ...
 *                    elected master.
 */
GceAutoscaleProvider.prototype.electMaster = function electMaster(instances) {
 ...
 */
GceAutoscaleProvider.prototype.getInstanceId = function getInstanceId() {
 ...
 *                   }
 */
GceAutoscaleProvider.prototype.getInstances = function getInstances(options) {
    ...
    return getMetadata('instance/zone')
        ...
        .then((instancesFromDb) => {
            ...
            instanceIdsInDb.forEach((instanceId) => {
                ...
                if (gceInstanceIds.indexOf(instanceId) !== -1) {
                    ...
                    // Get a list of non-master instances that we have in our db that GCE
        ...
        .then((missingInstances) => {
            ...
            logger.debug('Deleting non-masters that are not in GCE', idsToDelete);
 ...
 *                    }
 */
GceAutoscaleProvider.prototype.getMasterCredentials = function getMasterCredentials(mgmtIp, mgmtPort) {
    let credentials;
    let masterBigIp;
    ...
    return getData.call(this, CREDENTIALS_FILE)
        ...
        .catch((err) => {
            logger.info('Error getting master credentials', err);
            return q.reject(err);
        });
};
 ...
 *
 */
GceAutoscaleProvider.prototype.getMasterStatus = function getMasterStatus() {
    return getData.call(this, INSTANCES_FOLDER + this.instanceId)
        ...
        .catch((err) => {
            logger.info('Error getting master status', err);
            return q.reject(err);
        });
};
 ...
 *                    }
 */
GceAutoscaleProvider.prototype.getMessages = function getMessages(actions, options) {
    const promises = [];
    const subscriptions = [];
    ...
    actions.forEach((action) => {
        if (action === AutoscaleProvider.MESSAGE_SYNC_COMPLETE) {
            subscriptions.push(syncSubscriptionName);
        } else if (action === AutoscaleProvider.MESSAGE_ADD_TO_CLUSTER) {
 ...
 *                    is complete
 */
GceAutoscaleProvider.prototype.getPublicKey = function getPublicKey(instanceId) {
 ...
 */
GceAutoscaleProvider.prototype.getStoredUcs = function getStoredUcs() {
    const options = {
 ...
 */
GceAutoscaleProvider.prototype.masterElected = function masterElected(instanceId) {
    return setupTopicsAndSubscriptions.call(this, this.instanceId === instanceId)
        ...
        .then((instancesFromDb) => {
            ...
            instanceIdsInDb.forEach((dbInstanceId) => {
                ...
                if (dbInstanceId !== instanceId && instance.isMaster) {
        ...
        .catch((err) => {
            this.logger.info('masterElected error', err && err.message ? err.message : err);
            return q.reject(err);
        });
};
 ...
 * Called when a master has been elected.
 *
 * Update Virtual Machine instance Networks Tags, adding/removing the Deployment master tag.
 *
 ...
 * @returns {Promise} A promise which will be resolved when processing is complete.
 */
GceAutoscaleProvider.prototype.tagMasterInstance = function tagMasterInstance(masterIid, instances) {
    const vms = {};
    const masterTag = `${this.providerOptions.instanceGroup}-master`;
    ...
    return getMetadata('instance/zone')
        ...
        .then((responses) => {
            ...
            responses.forEach((response) => {
                ...
                const hasTag = tags.includes(masterTag);

                // If Master Instance, and instance is not tagged as master, tag it as master
                if (vmName === masterIid && (!hasTag)) {
                    tags.push(masterTag);
                    const vm = vms[vmName];
                    logger.debug(`Tagging Master Instance as: ${vmName}`);
                    promises.push(vm.setTags(tags, fingerprint));
                }
                // If non-Master Instance, and instance is tagged as master, remove master tag
                if (vmName !== masterIid && hasTag) {
                    tags.splice(tags.indexOf(masterTag), 1);
                    const vm = vms[vmName];
                    logger.debug(`Removing Master tag from: ${vmName}`);
                    promises.push(vm.setTags(tags, fingerprint));
                }
            });
            return q.all(promises);
 ...
 * @returns {Promise} A promise which will be resolved when processing is complete.
 */
GceAutoscaleProvider.prototype.masterInvalidated = function masterInvalidated(instanceId) {
 ...
 */
GceAutoscaleProvider.prototype.putInstance = function putInstance(instanceId, instance) {
    return putData.call(
        this,
 ...
 *                    is complete
 */
GceAutoscaleProvider.prototype.putMasterCredentials = function putMasterCredentials() {
    return bigIp.getPassword()
        ...
        .catch((err) => {
            return q.reject(new Error(`Unable to store master credentials: ${err}`));
        });
};
 ...
 *                    is complete
 */
GceAutoscaleProvider.prototype.putPublicKey = function putPublicKey(instanceId, publicKey) {
    return putData.call(
        this,
 ...
 */
GceAutoscaleProvider.prototype.sendMessage = function sendMessage(action, options) {
    const message = {
        action,
    ...
    if (action === AutoscaleProvider.MESSAGE_ADD_TO_CLUSTER) {
        topic = getJoinTopicName.call(this);
    } else if (action === AutoscaleProvider.MESSAGE_SYNC_COMPLETE) {
 ...
 */
GceAutoscaleProvider.prototype.storeUcs = function storeUcs(file, maxCopies, prefix) {
 ...
 */
GceAutoscaleProvider.prototype.syncComplete = function syncComplete(fromUser, fromPassword) {
 ...
 *     + subscribes to topics based on whether or not we are master
 *
 * @param {Boolean} isMaster - whether or not this instance is master
 *
 * @returns {Promise} A promise that is resolved when done or rejected
 *                    if an error occurs.
 */
function setupTopicsAndSubscriptions(isMaster) {
    ...
    return this.pubSub.getTopics()
        ...
        .then((data) => {
            ...
            // if we are master, subscribe to join requests
            if (isMaster && !hasJoinSubscription) {
            ...
            if (!isMaster && !hasSyncSubscritpion) {
